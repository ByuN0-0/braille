...
import data from "@/data/consonants-final.json";
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">종성(받침)</h1>
      ...
      {selected ? (
        <div className="sticky bottom-0 left-0 right-0 rounded-t-xl border p-4 bg-background">
          ...
          <div className="mt-3">
            <Link href={`/practice/input?section=consonants&id=${selected.id}`} className="px-3 py-2 rounded border inline-block">바로 연습</Link>
          </div>
        </div>
      ) : null}
    </div>
